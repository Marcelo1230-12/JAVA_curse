...
There are 4 popular way to write a file in Java

## File Writer 

This is an easy way to write small or medium sized of text files.
...
EX
    ...
    public static void main(String[] args) {
    // 'false' overwrites the file; use 'true' to append
    try (FileWriter writer = new FileWriter("output.txt", false)) {
    writer.write("Hello, this is a simple text file.\n");
    writer.write("Writing a second line directly.");
    } catch (IOException e) {
    e.printStackTrace();
            }
        }
...
This way has a better perfomance for larges amounts of texts.
...
EX.
    ...
    public static void main(String[] args) {
    try (BufferedWriter writer = new BufferedWriter(new FileWriter("large_log.txt"))) {
    for (int i = 0; i < 1000; i++) {
    writer.write("Log entry number: " + i);
    writer.newLine(); // Efficiently adds a platform-specific new line
        }
    // No need to manually flush() or close() due to try-with-resources
    } catch (IOException e) {
    e.printStackTrace();
                }
            }
...
This is the best for structures of data, perfect to write reports or logs
...
EX
    ...
    double salary = 75000.50;
            ...
            // Formatted output
            writer.printf("Employee Report:%n");
            writer.printf("Name: %s%n", name);
            writer.printf("Age: %d%n", age);
            writer.printf("Salary: $%,.2f%n", salary); // Formats number with commas and 2 decimals
...
This way is the perfect for binary files, like images and audio files.
...
EX.
    ...
    public static void main(String[] args) {
    // Example: Writing binary data (simulating an image header or raw bytes)
    byte[] data = { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A };

        try (FileOutputStream stream = new FileOutputStream("image.bin")) {
            stream.write(data);
            // You can also write single bytes: stream.write(255);
        } catch (IOException e) {
            e.printStackTrace();
            }
        }
